...
const db = {
  ...
  run: async (text, params = []) => {
    ...
    catch (e) { /* ignorar errores no críticos */ }
  },
  get: (text, params, cb) => {
...
// ─── RUTAS DE ARCHIVOS (seguras — no crashean si no existen) ──
...
// Solo crear UPLOADS_DIR si PUBLIC existe
...
// ─── SSR de precios (solo si los archivos existen localmente) ──
function serveSSR(htmlFile, priceMap, req, res) {
  if (!fs.existsSync(htmlFile)) {
    return res.status(404).json({ message: "Archivo no encontrado en este servidor" });
  }
...
function injectAndSaveHTML(filePath, priceMap, prices) {
  if (!fs.existsSync(filePath)) return; // silencioso si no existe
...
// Servir assets del backend/public (logo, etc)
if (fs.existsSync(BACKEND_PUBLIC)) {
  app.use(express.static(BACKEND_PUBLIC));
}

// Rutas del frontend — solo si existen localmente
if (fs.existsSync(PUBLIC_LANACIONAL)) {
...
app.post("/api/upload-image", auth, (req, res) => {
  if (!fs.existsSync(PUBLIC)) {
    return res.status(503).json({ message: "Almacenamiento de imágenes no disponible en este entorno" });
  }
...
app.put("/api/settings", auth, checkTimeRestrictionSelective, async (req, res) => {
  ...
  if (hasPrices && fs.existsSync(PUBLIC)) {
